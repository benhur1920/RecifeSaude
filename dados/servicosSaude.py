...
def criar_um_novo_DataFrame_com_as_colunas_desejadas_e_renomeando_as(df):
    df = df[['nome_oficial', 'endereço', 'bairro', 'fone', 'especialidade', 'horario', 'como_usar', 'latitude', 'longitude',
                       'Opcao']].rename(columns={'nome_oficial': 'Nome'})
    return df
...
def aplicando_capitalize_nos_nomes_das_colunas(dados):
    dados.columns = [col.capitalize() for col in dados.columns]
    return dados
...
def renomeando_coluna_opcao(df):
    df.rename(columns={"Opcao": "Opção"}, inplace=True)
    return df
...
def main():
    ...
    ubs = criar_a_coluna_opcao(ubs, 'USB')
    ...
    dados = aplicando_capitalize_nos_nomes_das_colunas(dados)
    dados = criar_a_coluna_Regiao(dados)
    dados = renomeando_coluna_opcao(dados)
    #salvar_o_DataFrame_no_arquivo_CSV_com_codificação_UTF_8_para_subir_para_o_BI(dados)

    return dados
# Gerando a variavel df para importar 
df = main()
